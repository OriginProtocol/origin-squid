...
const executeQuery = async (query: string) => {
  const response = await fetch('https://origin.squids.live/origin-squid@v82/api/graphql', {
    method: 'POST',
    headers: {
...
const main = async () => {
  ...
  const compare = (name: string, expected: bigint, actual: bigint) => {
    const diff = expected - actual
    const diffPercentage = (Number(diff) / Number(expected)) * 100
    console.log(`${name}: ${expected.toString()} ${actual.toString()} (${diffPercentage.toFixed(6)}%)`)
  }
  compare('Total Supply', totalSupply as bigint, BigInt(state.totalSupply))
  compare('Non-Rebasing Supply', nonRebasingSupply as bigint, BigInt(state.nonRebasingSupply))
