...
import { DOMAIN_SEPARATOR, accRewardPerShare, allowance, approve, asset, balanceOf, checkpoints, decimals, decreaseAllowance, delegates, epoch, getPastTotalSupply, getPastVotes, getVotes, increaseAllowance, lockups, lockupsCount, maxStakeDuration, minStakeDuration, name, nonces, numCheckpoints, previewPoints, previewRewards, previewWithdraw, rewardDebtPerShare, rewardsSource, symbol, totalSupply, transfer, transferFrom } from './functions.js'
import type { AllowanceParams, ApproveParams, BalanceOfParams, CheckpointsParams, DecreaseAllowanceParams, DelegatesParams, GetPastTotalSupplyParams, GetPastVotesParams, GetVotesParams, IncreaseAllowanceParams, LockupsCountParams, LockupsParams, NoncesParams, NumCheckpointsParams, PreviewPointsParams, PreviewRewardsParams, PreviewWithdrawParams, RewardDebtPerShareParams, TransferFromParams, TransferParams } from './functions.js'
...
export class Contract extends ContractBase {
    ...
    lockupsCount(user: LockupsCountParams["user"]) {
        return this.eth_call(lockupsCount, {user})
    }

    maxStakeDuration() {
        return this.eth_call(maxStakeDuration, {})
    }
    ...
    previewWithdraw(amount: PreviewWithdrawParams["amount"], end: PreviewWithdrawParams["end"]) {
        return this.eth_call(previewWithdraw, {amount, end})
    }
