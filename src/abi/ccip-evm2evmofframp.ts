...
export const events = {
    ...
    "ConfigSet((address,uint64,uint64,address,address,address),(uint32,address,address,uint16,uint32,uint32))": event("0xe668e1a4644c1a030b909bbfd837f5cfa914994ed5e0bb2e9c34a5c37753128a", {"staticConfig": p.struct({"commitStore": p.address, "chainSelector": p.uint64, "sourceChainSelector": p.uint64, "onRamp": p.address, "prevOffRamp": p.address, "armProxy": p.address}), "dynamicConfig": p.struct({"permissionLessExecutionThresholdSeconds": p.uint32, "router": p.address, "priceRegistry": p.address, "maxNumberOfTokensPerMsg": p.uint16, "maxDataBytes": p.uint32, "maxPoolReleaseOrMintGas": p.uint32})}),
    "ConfigSet(uint32,bytes32,uint64,address[],address[],uint8,bytes,uint64,bytes)": event("0x1591690b8638f5fb2dbec82ac741805ac5da8b45dc5263f4875b0496fdce4e05", {"previousConfigBlockNumber": p.uint32, "configDigest": p.bytes32, "configCount": p.uint64, "signers": p.array(p.address), "transmitters": p.array(p.address), "f": p.uint8, "onchainConfig": p.bytes, "offchainConfigVersion": p.uint64, "offchainConfig": p.bytes}),
    ExecutionStateChanged: event("0xd4f851956a5d67c3997d1c9205045fef79bae2947fdee7e9e2641abc7391ef65", {"sequenceNumber": indexed(p.uint64), "messageId": indexed(p.bytes32), "state": p.uint8, "returnData": p.bytes}),
    ...
    SkippedIncorrectNonce: event("0xd32ddb11d71e3d63411d37b09f9a8b28664f1cb1338bfd1413c173b0ebf41237", {"nonce": indexed(p.uint64), "sender": indexed(p.address)}),
    SkippedSenderWithPreviousRampMessageInflight: event("0xe44a20935573a783dd0d5991c92d7b6a0eb3173566530364db3ec10e9a990b5d", {"nonce": indexed(p.uint64), "sender": indexed(p.address)}),
    Transmitted: event("0xb04e63db38c49950639fa09d29872f21f5d49d614f3a969d8adf3d4b52e41a62", {"configDigest": p.bytes32, "epoch": p.uint32}),
}
...
export const functions = {
    ...
    ccipReceive: fun("0x85572ffb", {"_0": p.struct({"messageId": p.bytes32, "sourceChainSelector": p.uint64, "sender": p.bytes, "data": p.bytes, "destTokenAmounts": p.array(p.struct({"token": p.address, "amount": p.uint256}))})}, ),
    ...
    executeSingleMessage: fun("0xf52121a5", {"message": p.struct({"sourceChainSelector": p.uint64, "sender": p.address, "receiver": p.address, "sequenceNumber": p.uint64, "gasLimit": p.uint256, "strict": p.bool, "nonce": p.uint64, "feeToken": p.address, "feeTokenAmount": p.uint256, "data": p.bytes, "tokenAmounts": p.array(p.struct({"token": p.address, "amount": p.uint256})), "sourceTokenData": p.array(p.bytes), "messageId": p.bytes32}), "offchainTokenData": p.array(p.bytes)}, ),
    getDestinationToken: fun("0xb4069b31", {"sourceToken": p.address}, p.address),
    getDestinationTokens: fun("0x681fba16", {}, p.array(p.address)),
    getDynamicConfig: fun("0x7437ff9f", {}, p.struct({"permissionLessExecutionThresholdSeconds": p.uint32, "router": p.address, "priceRegistry": p.address, "maxNumberOfTokensPerMsg": p.uint16, "maxDataBytes": p.uint32, "maxPoolReleaseOrMintGas": p.uint32})),
    getExecutionState: fun("0x142a98fc", {"sequenceNumber": p.uint64}, p.uint8),
    getPoolByDestToken: fun("0xd7e2bb50", {"destToken": p.address}, p.address),
    getPoolBySourceToken: fun("0x5d86f141", {"sourceToken": p.address}, p.address),
    getSenderNonce: fun("0x856c8247", {"sender": p.address}, p.uint64),
    getStaticConfig: fun("0x06285c69", {}, p.struct({"commitStore": p.address, "chainSelector": p.uint64, "sourceChainSelector": p.uint64, "onRamp": p.address, "prevOffRamp": p.address, "armProxy": p.address})),
    getSupportedTokens: fun("0xd3c7c2c7", {}, p.array(p.address)),
    getTokenLimitAdmin: fun("0x599f6431", {}, p.address),
    getTransmitters: fun("0x666cab8d", {}, p.array(p.address)),
    latestConfigDetails: fun("0x81ff7048", {}, {"configCount": p.uint32, "blockNumber": p.uint32, "configDigest": p.bytes32}),
    latestConfigDigestAndEpoch: fun("0xafcb95d7", {}, {"scanLogs": p.bool, "configDigest": p.bytes32, "epoch": p.uint32}),
    manuallyExecute: fun("0x740f4150", {"report": p.struct({"messages": p.array(p.struct({"sourceChainSelector": p.uint64, "sender": p.address, "receiver": p.address, "sequenceNumber": p.uint64, "gasLimit": p.uint256, "strict": p.bool, "nonce": p.uint64, "feeToken": p.address, "feeTokenAmount": p.uint256, "data": p.bytes, "tokenAmounts": p.array(p.struct({"token": p.address, "amount": p.uint256})), "sourceTokenData": p.array(p.bytes), "messageId": p.bytes32})), "offchainTokenData": p.array(p.array(p.bytes)), "proofs": p.array(p.bytes32), "proofFlagBits": p.uint256}), "gasLimitOverrides": p.array(p.uint256)}, ),
    owner: fun("0x8da5cb5b", {}, p.address),
    setAdmin: fun("0x704b6c02", {"newAdmin": p.address}, ),
    setOCR2Config: fun("0x1ef38174", {"signers": p.array(p.address), "transmitters": p.array(p.address), "f": p.uint8, "onchainConfig": p.bytes, "offchainConfigVersion": p.uint64, "offchainConfig": p.bytes}, ),
    ...
    transmit: fun("0xb1dc65a4", {"reportContext": p.fixedSizeArray(p.bytes32, 3), "report": p.bytes, "rs": p.array(p.bytes32), "ss": p.array(p.bytes32), "_4": p.bytes32}, ),
