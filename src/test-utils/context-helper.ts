...
import { OTokenDailyStat, ProcessingStatus, ProtocolDailyStatDetail, StrategyBalance } from '@model'
...
export const createMockProtocolDailyStatDetail = (
  overrides: Partial<ProtocolDailyStatDetail> = {},
...
export const setupMockStore = (
  ...
  mockStore.findOne.callsFake((entity: any, options: any) => {
    if (entity === OTokenDailyStat) {
      return (
        data.otokenDailyStats?.find(
          (stat) => stat.date === options.where?.date && stat.otoken === options.where?.otoken,
        ) || null
      )
    }
    ...
    if (entity === StrategyBalance) {
      ...
      if (options.where?.timestamp && typeof options.where.timestamp === 'object') {
        return (
          data.strategyBalances?.find(
            (balance) =>
              balance.strategy === options.where.strategy && balance.timestamp <= options.where.timestamp.value,
          ) || null
        )
      }
      return data.strategyBalances?.find((balance) => balance.strategy === options.where?.strategy) || null
    }
    ...
    if (entity === ProtocolDailyStatDetail) {
      if (options.where?.date && options.where?.product) {
        ...
        const result =
          data.protocolDailyStatDetails?.find(
            (detail) => detail.date === options.where.date && detail.product === options.where.product,
          ) || null
        return result
      } else if (options.where?.product && options.order?.date === 'desc') {
        // Query for latest by product (getLatestProtocolDailyStatDetail)
        const matches =
          data.protocolDailyStatDetails?.filter((detail) => detail.product === options.where.product) || []
    ...
    if (entity === ProcessingStatus) {
      const result = data.processingStatuses?.find((status) => status.id === options.where?.id) || null
      return result
    }
  ...
  mockStore.find.callsFake((entity: any, options: any) => {
    if (entity === OTokenDailyStat && options.where?.date?._type === 'in') {
      ...
      const result =
        data.otokenDailyStats?.filter((stat) => dates.includes(stat.date) && stat.otoken === options.where.otoken) || []
      return result
    }

    if (entity === ProtocolDailyStatDetail && options.where?.date?.constructor.name === 'In') {
      const dates = options.where.date.value
      return data.protocolDailyStatDetails?.filter((detail) => dates.includes(detail.date)) || []
    }
