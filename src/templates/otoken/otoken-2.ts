      ...
      for (const block of ctx.blocks) {
        ...
        for (const transaction of block.transactions) {
          ...
          const transferLogs = transaction.logs
            ...
            .map((log) => ({ ...log, decoded: otokenAbi.events.Transfer.decode(log) }))
              ...
              if (proxyInitializeTraceFilter.matches(trace)) {
                ...
                const transferLog = transferLogs.find(
                  (log) =>
                    ...
                    log.decoded.from.toLowerCase() === sender &&
                    log.decoded.to.toLowerCase() === data._to.toLowerCase(),
                )
                if (transferLog) {
                  consumedLogs.add(transferLog.id)
                ...
                const transferLog = transferLogs.find(
                  (log) =>
                    !consumedLogs.has(log.id) &&
                    log.decoded.from.toLowerCase() === data._from.toLowerCase() &&
                    log.decoded.to.toLowerCase() === data._to.toLowerCase(),
                )
                if (transferLog) {
                  consumedLogs.add(transferLog.id)
