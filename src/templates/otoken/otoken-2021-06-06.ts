...
export class OToken_2021_06_06 {
  ...
  public copyState(other: OToken_2021_01_25): void {
  ...
  private _ensureRebasingMigration(account: string): void {
    const nonRebasingCredits = this.nonRebasingCreditsPerToken[account]
    if (!nonRebasingCredits || nonRebasingCredits === 0n) {
      if ((this.creditBalances[account] ?? 0n) == 0n) {
        // Since there is no existing balance, we can directly set to
        // high resolution, and do not have to do any other bookkeeping
        this.nonRebasingCreditsPerToken[account] = 10n ** 27n
      } else {
        ...
        this.rebasingCredits = this.rebasingCredits - (this.creditBalances[account] ?? 0n)
      }
    }
  }
