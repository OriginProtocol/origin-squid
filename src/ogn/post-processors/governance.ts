...
const _updateProposalStatus = async (
  ...
  if (
    proposal.status === OGNProposalState.Pending &&
    !pendingProposals.find((p) => p.id === proposal.id)
  ) {
    pendingProposals.push(proposal)
  }
  if (
    proposal.status === OGNProposalState.Active &&
...
const _processVoteCast = async (
  ...
  const voteType = [OGNVoteType.Against, OGNVoteType.For, OGNVoteType.Abstain][
