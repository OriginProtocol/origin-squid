...
import { OGN, OGNAddress, OGNDailyStat } from '@model'
...
export const process = async (ctx: Context) => {
  ...
  const dailyStats = [] as OGNDailyStat[]
...
async function updateDailyStats(ctx: Context, date: Date) {
  ...
  const [lastOgn, holdersOverThreshold] = await Promise.all([
    ctx.store.findOne(OGN, queryParams),
    ctx.store.countBy(OGNAddress, { balance: MoreThanOrEqual(10n ** 20n) }), // 100 OGN
  ])

  const allEntities = [lastOgn].filter(Boolean)
  if (!lastOgn) {
    return null
  }
  ...
  const dailyStat = new OGNDailyStat({
    ...
    totalSupply: lastOgn?.total || 0n,
    totalStaked: lastOgn?.staked,
    totalSupplyUSD: 0,
    tradingVolumeUSD: 0,
    marketCapUSD: 0,
    priceUSD: 0,
