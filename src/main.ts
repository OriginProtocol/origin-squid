...
processor.run(new TypeormDatabase({ supportHotBlocks: true }), async (ctx) => {
  ...
  const { history, rebases, owners, vaults } = await transform(ctx, logs)

  const ownerValues = [...owners.values()]
  ctx.log.info(
    `Storing: ${ownerValues.length} owners, ${history.length} histories, ${rebases.length} rebases, ${vaults.length} vaults`,
  )
